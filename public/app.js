...
function buildList(key){const gs=edpGroups(key);let rows='';gs.forEach(g=>g.lines.forEach(l=>{rows+=`<tr><td><input type="checkbox" aria-label="select ${l.nm}"></td><td><b>${l.nm}</b></td><td>${l.qty}</td><td><span class="grp-chip" style="background:${grpColorVar(g.cls)}1a;color:${grpColorVar(g.cls)}">${g.key}</span></td><td>${MONTHS[l.start]} 2026</td><td>${MONTHS[Math.min(7,l.start+l.span-1)]}</td><td class="mono">${l.code}</td><td>${l.cost}</td><td><span class="chip gray">Planned</span></td></tr>`;}));return `<div class="list"><table class="mini" style="padding:4px 10px"><tr><th style="width:26px"></th><th>Equipment</th><th>Qty</th><th>${key}</th><th>On-rent</th><th>Off-rent</th><th>Cost code</th><th>Est. cost</th><th>Status</th></tr>${rows}</table></div><div style="display:flex;gap:8px;margin-top:12px"><button class="btn primary" onclick="promoteSelected()">${ic('send','ic-14')} Promote selected → request</button><button class="btn" onclick="toast('Off-rent request initiated for selected lines')">Initiate off-rent</button></div>`;}
...
let NSWALK_STEP=0;
const NSWALK=[
 ['Read your project',ic('team','ic-20'),'I pulled Hercules Solar + BESS from CRM — a ~$180M solar + BESS build in Hercules, CA, currently in preconstruction. That tells me the shape of the equipment you\u2019ll need.'],
 ['Read your schedule',ic('calendar','ic-20'),'I used Scheduling Intelligence data in the Ontology — anchored in your P6 schedule — to find the activities across sitework, foundations, steel, racking, MEP, and commissioning, and set every on-rent / off-rent window from their dates.'],
 ['Read your estimate',ic('clipboard','ic-20'),'Estimating Intelligence, based in Foundry, gave me cost codes and budgets by scope. I divided each code\u2019s budget by the equipment rate to get billable months — the same math your team does by hand today.'],
 ['Compared 5 similar jobs',ic('gauge','ic-20'),'I matched against 5 completed solar + BESS builds to fill gaps and set quantities — e.g. 92% of them ran 2 large excavators through grading and 6 scissor lifts through racking.'],
 ['Built the plan',ic('sparkle','ic-20'),'The result is a full draft across activity groups, already coded, dated, and grouped by the activity it supports. Review the Gantt, adjust anything, or accept as-is.']
];
...
function promoteSelected(){openConfirm('Promote to request?','Selected planned lines will be promoted into a single request carrying those lines as embedded structure — one push to Command Center, no re-entry. Feasibility of batch promotion is confirmed post writeback-API.','Promote → request',()=>toast('Lines promoted into REQ-1059 · sent to Command Center'));}
...
function edpSendRequest(){if(EDP_CART===0){toast('Cart is empty — add lines from the plan first');return;}
 openModal(`<div class="modal-head"><div><h3>Send request</h3><div class="sub">${EDP_CART} line${EDP_CART>1?'s':''} · one call-on to 02S</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body"><div class="valid-list"><div class="vrow ok">${ic('check','ic-16')}<div><b>${EDP_CART} planned line${EDP_CART>1?'s':''} in the cart</b><div class="vh">Promoted straight from the plan — no re-entry</div></div></div><div class="vrow ok">${ic('check','ic-16')}<div><b>On-rent / off-rent dates set</b><div class="vh">From the plan window (drag-adjustable)</div></div></div></div>
   <div class="oc-edit"><div class="oc-lbl">${ic('edit','ic-14')} Cost code — edit before sending <span class="req">required</span></div><div class="oc-row"><input id="edpCode" value="010200.3005.2000" class="mono" oninput="edpCodeCheck()"><span class="code-chip" id="edpCodePrev">010200.3005.2000</span><span id="edpCodeStatus"><span class="chip ok">${ic('check','ic-14')} 16/16</span></span></div><div class="oc-help">Controlling code + CMiC task code applied to every line in this request. Edit the Job / Expenditure / Task segments as needed before it goes to 02S.</div></div>
   <div class="route-note" style="margin-top:12px">This becomes one request (call-on) to 02S ops. Equipment routes to YardHub; you'll see status write back here.</div></div><div class="modal-foot"><button class="btn" onclick="closeModal()">Keep editing</button><button class="btn primary" id="edpSendBtn" onclick="EDP_CART=0;edpCartBadges();closeModal();toast('Request sent to 02S — status will write back')">${ic('send','ic-14')} Send ${EDP_CART} to 02S</button></div>`,true);}
...
function openCartDrawer(){openModal(`<div class="modal-head"><h3>${ic('cart','ic-16')} Your order <span class="cd-count">${cartCount()} ${cartCount()===1?'item':'items'}</span></h3><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body" id="cartDrawerBody">${cartDrawerLinesHTML()}<div class="cd-total"><span>Estimated /mo</span><b>${cartDrawerTotal()}</b></div><div class="cd-note">${ic('bulb','ic-14')} One consolidated order across the Product catalog and On-demand browse &amp; checkout. Dates, delivery address, and validation are set at checkout.</div></div><div class="modal-foot"><button class="btn" onclick="closeModal()">Keep browsing</button><button class="btn primary" onclick="closeModal();nav('portal','p-browse')"${BROWSE_CART.length?'':' disabled'}>${ic('check','ic-14')} Go to checkout</button></div>`);}
function cartDrawerRefresh(){const b=document.getElementById('cartDrawerBody');if(b)b.innerHTML=`${cartDrawerLinesHTML()}<div class="cd-total"><span>Estimated /mo</span><b>${cartDrawerTotal()}</b></div><div class="cd-note">${ic('bulb','ic-14')} One consolidated order across the Product catalog and On-demand browse &amp; checkout. Dates, delivery address, and validation are set at checkout.</div>`;const c=document.querySelector('.cd-count');if(c)c.textContent=cartCount()+' '+(cartCount()===1?'item':'items');}
...
function renderBacklog(f){const el=document.getElementById('backlogRows');if(!el)return;const pillars=f==='All'?PILLARS:[f];const ns=effMode('c-backlog')==='northstar';
 ...
 pillars.forEach(p=>{const rows=BACKLOG.filter(r=>r.pillar===p);if(!rows.length)return;const nNew=rows.filter(r=>r.status==='New').length;const eq=p==='Equipment';
   const route=ns?`${ic('sparkle','ic-14')} Routed via T3 (OMS) · live status`:(eq?'Auto-routes to YardHub':'Managed queue · fulfill outside of Command Center and update status manually');
   h+=`<div class="back-group"><div class="bg-head"><span class="pdot" style="background:${pillColor(p)}"></span><b>${p}</b><span class="bg-count">${rows.length} open${nNew?` · ${nNew} new`:''}</span><span class="bg-route">${route}</span></div><div class="list"><div class="lrow cols-back lhead"><div>Ref</div><div>Item · project</div><div style="text-align:center">Qty</div><div>Need by</div><div>Origin</div><div>Status</div><div></div></div>${rows.map(backlogRowHTML).join('')}</div></div>`;});
...
const ORDERS=[
 {id:'ORD-3042',prod:'2× ¾-Ton 4×4',placed:'May 12',arrived:'—',status:'In fulfillment',code:'010200.3005.2000',dates:'May 12 – Aug 28',stage:2,pillar:'Equipment'},
...
function openOrderDetail(i){const o=ORDERS[i];const ns=effMode('p-orders')==='northstar';
 ...
 <div class="modal-foot">${ns?`<button class="btn" onclick="toast('Receiving confirmed · photos attached')">${ic('check','ic-14')} Confirm receipt</button><button class="btn" onclick="toast('Off-rent initiated')">Initiate off-rent</button>`:''}<button class="btn primary" onclick="closeModal()">Close</button></div>`,true);}
...
function commandPillarPage(key){const p=CPILLAR[key];const c=p.color;const sid='c-pill-'+key;
 ...
 <div class="card" style="margin:16px 0"><div class="ch"><span class="ci" style="background:${c}1a;color:${c}">${ic('inbox','ic-16')}</span><span class="t">${p.name} demand across projects</span><span class="sub">select a row to review &amp; fulfill</span></div>
   ...
   ${p.rows.map((r,i)=>`<div class="lrow" style="grid-template-columns:1.8fr 1.6fr .7fr .8fr 1fr 1.1fr"><div class="pri"><span class="pdot" style="background:${c}"></span>${r[0]}</div><div style="color:var(--gray-500)">${r[1]}</div><div>${r[2]}</div><div>${r[3]}</div><div><span class="chip ${r[4]==='New'?'gray':'warn'}">${r[4]}</span></div><div style="display:flex;gap:6px;justify-content:flex-end"><button class="btn ghost sm" onclick="toast('Status updated · project notified')">Update</button><button class="btn primary sm" onclick="toast('Fulfillment started')">Fulfill</button></div></div>`).join('')}</div>
 </div>
 <div class="card" style="margin-bottom:16px"><div class="ch"><span class="ci k2">${ic('calendar','ic-16')}</span><span class="t">Portfolio demand — projected + in-flight</span><span class="sub">by ${p.name.toLowerCase()} category / project</span></div>
   ...
   <div style="font-size:11px;color:var(--gray-500);margin-top:10px">Demand is projected from the pipeline and rolled up from live requests — the same granular signal 02S gets for equipment, now sourced from OMS (T3) across every pillar.</div>
 </div>
...
function pillarPlan(id,cfg){const m=effMode(id);const pc=pillColor(cfg.name==='Prof. services'?'Professional services':cfg.name);
 ...
 return `<div class="page-head"><div><h2>${cfg.name} plan</h2><div class="desc">02S drafted this from your schedule, estimate, and CRM — review, adjust, and accept.</div></div>${prodToggle(id)}</div>
...
'p-dash':(id)=>`
 <div class="page-head"><div><h2>Project team</h2><div class="desc">Orders, rentals, demand, and spend — Hercules Solar + BESS</div></div>${prodToggle(id)}</div>
 <div class="v1-only">
 ...
 <div class="ns-only">
   <div class="card span3" style="margin-bottom:16px"><div class="ch"><span class="ci">${ic('calendar','ic-16')}</span><span class="t">3-week 02S lookahead</span><span class="sub">execution + 02S touchpoints, tied to the schedule</span><span class="ns-badge" style="margin-left:auto">North Star</span></div>
     ...
     <div class="la-touch">
       <div class="lt-col"><div class="lt-h">This week</div>
         ...
         <div class="lt-item act"><span class="lt-ico" style="background:var(--red-050);color:var(--red)">${ic('clock','ic-14')}</span><div><b>2× scissor lift idle 6 days</b><span class="lt-m">low usage · $1.9K/wk exposure</span></div><button class="btn sm" onclick="toast('Off-rent initiated')">Off-rent</button></div>
       </div>
       <div class="lt-col"><div class="lt-h">Next week</div>
         <div class="lt-item act"><span class="lt-ico" style="background:var(--prefab-050);color:var(--prefab)">${ic('file','ic-14')}</span><div><b>Prefab submittal due</b><span class="lt-m">L2 headwall shop drawings · Thu</span></div><button class="btn sm" onclick="toast('Opened')">Open</button></div>
       ...
       <div class="lt-col"><div class="lt-h">In 2 weeks</div>
         <div class="lt-item act"><span class="lt-ico" style="background:var(--red-050);color:var(--red)">${ic('truck','ic-14')}</span><div><b>Tower crane mobilization</b><span class="lt-m">Structure phase · add ship-to?</span></div><button class="btn sm" onclick="toast('Ship-to added → OMS')">Add</button></div>
         <div class="lt-item"><span class="lt-ico" style="background:var(--gold-050);color:var(--gold)">${ic('warning','ic-14')}</span><div><b>Safety bulletin — crane lift zone</b><span class="lt-m">acknowledge before mob</span></div><button class="btn sm" onclick="toast('Acknowledged')">Ack</button></div>
       </div>
     </div>
...
'p-catalog':(id)=>`
 <div class="page-head"><div><h2>Product catalog</h2><div class="desc v1-only">Browse 02S products & services by pillar. Coverage varies by pillar in V1.</div><div class="desc ns-only">One full catalog across every pillar — specific offerings with pricing, specs, and lead times.</div></div><div class="ph-actions">${cartButton()}${prodToggle(id)}</div></div>
...
'p-orders':(id)=>`
 <div class="page-head"><div><h2>Orders</h2><div class="desc">Active orders and complete order history — click any order for full detail.</div></div>${prodToggle(id)}</div>
...
'p-edp':(id)=>`
 <div class="page-head"><div><h2>Equipment demand plan</h2><div class="desc v1-only">Plan the equipment this job needs, then submit requests against the plan.</div><div class="desc ns-only">02S already built this from your schedule, estimate, and CRM — review the walkthrough and confirm.</div></div>${prodToggle(id)}</div>
...
'p-support':(id)=>`
 <div class="page-head"><div><h2>Contact & support</h2><div class="desc">Reach 02S from inside the portal.</div></div>${prodToggle(id)}</div>
 <div class="v1-only">
...
'c-dash':(id)=>`
 ...
 <div class="ns-only"><div class="mesh-card"><div class="mesh-head"><div class="mesh-title"><span class="ci" style="background:rgba(255,255,255,.15);color:#fff">${ic('gauge','ic-16')}</span><div><div class="mh-t">What's connected across 02S right now</div><div class="mh-s">The signals from each pillar don't live in isolation — here's where they add up to something worth acting on</div></div></div><span class="illus-tag" style="background:rgba(255,255,255,.15);color:#fff;border-color:rgba(255,255,255,.3)">Illustrative</span></div>
   <div class="mesh-signals">${[
     ['Equipment','var(--red)','truck','Tower-crane shortfall wk of Aug 3','−2 units vs. demand'],
     ...
     ['Logistics','var(--logistics)','map','Oakland delivery density rising','warehouse case building'],
   ...
   <div class="mesh-synth"><div class="msy-h">${ic('bulb','ic-16')} Where this adds up</div>
     <div class="msy-list">
       <div class="msy-row"><span class="msy-tag">Mercy Hospital</span><div><b>Steel, crane, and commissioning all land in the same window.</b> The August crane shortfall, the Tucson shop running hot, and commissioning already booked for November all hit this one job — a real chance of tripping over ourselves. Worth pulling the crane forward and moving the headwall pre-builds to Phoenix.</div></div>
       <div class="msy-row"><span class="msy-tag">Portfolio</span><div><b>A Bay Area yard would pay for itself.</b> Deliveries are clustering around Oakland and we keep missing on EV-charger lead times — a laydown there cuts miles and lets us stage long-lead buys closer to the jobs.</div></div>
       <div class="msy-row"><span class="msy-tag">Margin</span><div><b>About $26K is sitting on the table this month.</b> Two units are billing while idle, and one re-rent is running 12% over what the owned equivalent would cost.</div></div>
     </div>
     <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap"><button class="btn primary sm" onclick="showActionPlan()">${ic('send','ic-14')} Turn into an action plan</button><button class="btn ghost sm" onclick="nav('command','c-pill-log')">See the yard case ${ic('chevronRight','ic-14')}</button></div>
   </div>
 </div></div>
 ...
 <div class="grid g3" style="margin-bottom:16px">
   <div class="v1-only" style="--disp1:flex"><div class="card"><div class="ch"><span class="ci">${ic('calendar','ic-16')}</span><span class="t">3-week execution lookahead</span></div><table class="mini"><tr><td>Earthwork</td><td><span class="chip gray">wk 1–2</span></td></tr><tr><td>Structural steel</td><td><span class="chip gray">wk 1–3</span></td></tr><tr><td>Mechanical</td><td><span class="chip gray">wk 2–3</span></td></tr></table></div></div>
 ...
 <div class="grid g2" style="margin-bottom:16px">
   ...
   <div class="card"><div class="ch"><span class="ci k2">${ic('warning','ic-16')}</span><span class="t">Exceptions &amp; SLA risk</span><span class="sub">open items</span></div><table class="mini"><tr><th>Item</th><th>Project</th><th>Risk</th></tr><tr><td>Tower crane shortfall</td><td>Alpha</td><td><span class="chip red">High</span></td></tr><tr><td>Idle rental &gt;7 days</td><td>Beta</td><td><span class="chip warn">Med</span></td></tr><tr><td>Late equipment delivery</td><td>Gamma</td><td><span class="chip gray">Low</span></td></tr></table><button class="clink" onclick="nav('command','c-billing')">Review in billings &amp; invoicing ${ic('chevronRight','ic-14')}</button></div>
 </div>
...
'c-backlog':(id)=>`
 <div class="page-head"><div><h2>Order backlog</h2><div class="desc v1-only">Every submitted order and request across all five pillars — grouped by pillar so you can see what's coming and work each queue.</div><div class="desc ns-only">A live tracking dashboard — every request routes through T3 (OMS), and statuses flow back here automatically across all five pillars.</div></div>${prodToggle(id)}</div>
 <div class="ns-only">
   <div class="oms-bar"><div class="oms-l"><span class="ci" style="background:rgba(255,255,255,.15);color:#fff">${ic('sparkle','ic-16')}</span><div><div class="oms-t">Everything routes through T3 (OMS)</div><div class="oms-s">Requests are triaged, coded, and fulfilled by OMS — statuses sync back here live. Nothing is updated by hand; this is a tracking view.</div></div></div>
...
'c-edp':(id)=>`
 ...
 <div class="ns-only">
   ...
   <div class="card" style="margin-bottom:16px"><div class="ch"><span class="ci">${ic('box','ic-16')}</span><span class="t">Consolidated orders</span><span class="sub">across all projects · fulfilled by OMS</span></div>
     ...
     <div style="font-size:11px;color:var(--gray-500);margin-top:10px">In North Star there's no YardHub hand-off and no taxonomy mapping by ops — OMS classifies, sources owned-vs-re-rent, codes, and dispatches on its own. This view is for visibility and exceptions only.</div>
   </div>
 </div>
 ...
 <div class="gantt2"><div class="g2-row head"><div class="g2-left"><span class="sub" style="font-weight:700;color:var(--gray-500)">Cat class / project</span></div><div class="g2-track">${MONTHS.map(m=>`<span>${m}</span>`).join('')}</div><div class="g2-cost" style="font-size:10px;color:var(--gray-500)">Count</div></div>
   ${[['Tower cranes',0,0,6,'14','group'],['— Project Alpha',1,10,45,'2',''],['— Project Nexus',2,25,45,'3',''],['Excavators 45–55T',0,0,88,'9','group'],['Scissor lifts',0,50,45,'22','group']].map(r=>`<div class="g2-row ${r[5]==='group'?'group':'line'}"><div class="g2-left"><span class="nm">${r[0]}</span></div><div class="g2-track">${r[5]==='group'?`<div class="gbar gbar-agg n2" style="left:${r[2]}%;width:${r[3]-2}%"></div>`:`<div class="gbar n2" style="left:${r[2]}%;width:${r[3]-2}%"></div>`}</div><div class="g2-cost">${r[4]}</div></div>`).join('')}
 </div>
 <div style="font-size:11px;color:var(--gray-500);margin-top:10px">Rolls projected + in-flight demand up across all projects by equipment class — drill into a class to see contributing projects.</div></div>
 ...
 <div class="ns-only" style="margin-top:16px">
   <div class="card" style="margin-bottom:16px"><div class="ch"><span class="ci">${ic('sparkle','ic-16')}</span><span class="t">Pipeline-predicted demand</span><span class="sub">tower cranes · next 6 months</span><span class="ns-badge" style="margin-left:auto">North Star</span></div>
     <p style="color:var(--charcoal-700);font-size:12.5px;margin:0 0 12px">Beyond what's booked, 02S projects demand from the opportunity pipeline — weighting soft-award and probable jobs by win %. The band widens with uncertainty; sustained gaps feed the CAPEX plan.</p>
   ...
   <div class="card"><div class="ch"><span class="ci k4">${ic('gauge','ic-16')}</span><span class="t">Fleet-wide optimization</span><span class="sub">owned utilization across all projects</span><span class="ns-badge" style="margin-left:auto">North Star</span></div>
     <p style="color:var(--charcoal-700);font-size:12.5px;margin:0 0 12px">02S balances the owned fleet across every active job — surfacing idle assets to redeploy before re-renting, and flagging classes running hot.</p>
...
'c-pillars':(id)=>`
 ...
 <div class="v1-only"><div class="limit-note" style="background:var(--steel-050);border-color:#CFE0EF">${ic('layers','ic-16')}<div><b>V1 is deliberately simple for the other pillars:</b> see the demand the projects have submitted, and manually update the status of each request as your team works it. No planning engine, no optimization — that comes in North Star.</div></div>
