...
const EQLINES=[
 {nm:'Excavator 45–55T',qty:2,cat:'Earthmoving',phase:'Sitework & Civil',code:'31210000.3005.2000',start:0,span:3,cost:'$101K',cls:''},
 {nm:'Wheel loader 3yd',qty:1,cat:'Earthmoving',phase:'Sitework & Civil',code:'31220000.3005.2000',start:2,span:4,cost:'$24K',cls:''},
 {nm:'Water truck 4K',qty:1,cat:'Support',phase:'Sitework & Civil',code:'31230000.3005.2000',start:0,span:5,cost:'$31K',cls:''},
 {nm:'Tower crane',qty:1,cat:'Lifting',phase:'Structure',code:'05100000.3005.2000',start:2,span:2,cost:'$318K',cls:'n2'},
 {nm:'Telehandler 10K',qty:2,cat:'Lifting',phase:'Structure',code:'05120000.3005.2000',start:3,span:4,cost:'$86K',cls:'n2'},
 {nm:'Scissor lift 32ft',qty:3,cat:'Aerial',phase:'MEP / Electrical',code:'26260000.3005.2000',start:4,span:2,cost:'$37K',cls:'n3'},
 {nm:'Boom lift 60ft',qty:2,cat:'Aerial',phase:'MEP / Electrical',code:'26261000.3005.2000',start:5,span:2,cost:'$52K',cls:'n3'}
];
  ...
  return `<div style="display:flex;flex-direction:column;gap:18px">
    ${data.map(d=>`<div style="display:flex;align-items:flex-end;gap:14px">
      ...
      <div style="display:flex;gap:3px;align-items:flex-end;flex:1">
        ${d.monthly.map((v,m)=>{const ht=Math.round(v/globalMax*52);const isPeak=v===d.peak&&d.peak>0;return `<div style="display:flex;flex-direction:column;align-items:center;gap:2px;flex:1"><div style="height:12px;display:flex;align-items:flex-end;justify-content:center">${isPeak?`<span style="font-size:8px;color:var(--red);font-weight:700;line-height:1">${v}</span>`:''}</div><div style="height:${ht||0}px;min-height:${v>0?3:0}px;background:${isPeak?'var(--red)':v>0?'var(--gray-300)':'transparent'};border-radius:3px 3px 0 0;width:100%" title="${v} unit${v!==1?'s':''} · ${MONTHS[m]}"></div><span style="font-size:9px;color:var(--gray-400)">${MONTHS[m]}</span></div>`;}).join('')}
      </div>
      <div style="text-align:right;flex-shrink:0;padding-bottom:20px;min-width:72px"><div style="font-size:20px;font-weight:700;line-height:1;color:${d.peak>1?'var(--charcoal)':'var(--gray-400)'}">${d.peak}</div><div style="font-size:10px;color:var(--gray-500)">peak · ${MONTHS[d.peakMonth]}</div></div>
    </div>`).join('')}
...
function promoteSelected(){
  ...
  // R-20: validate — block if any selected line has no on-rent date set (start date required)
  const missing=rows.filter(r=>!r.dates||r.dates==='Per plan window');
  if(missing.length&&checked.length){openModal(`<div class="modal-head"><div><h3>Cannot promote</h3><div class="sub">Validation failed · ${missing.length} line${missing.length!==1?'s':''} missing dates</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body"><div class="co-note" style="color:var(--red)">${ic('warning','ic-14')} Each line must have an on-rent date before it can be promoted to a request. Set dates on the plan and try again.</div><div class="list" style="margin-top:12px">${missing.map(r=>`<div class="lrow" style="grid-template-columns:1fr auto"><div>${r.name}</div><div><span class="chip red">Missing dates</span></div></div>`).join('')}</div></div><div class="modal-foot"><button class="btn primary" onclick="closeModal()">Go back &amp; fix</button></div>`);return;}
...
function openRecertModal(){
  const items=[
    {id:'ORD-3042',item:'2× ¾-Ton 4×4',on:'May 20',code:'13130000.3005.2000',note:'Civil support — active use by site crew'},
    {id:'ORD-3044',item:'1× tower crane (self-erect)',on:'Aug 3',code:'05100000.3005.2000',note:'Structure phase — still in use through Sep'},
    {id:'ORD-3047',item:'3× scissor lift 32ft',on:'May 1',code:'26260000.3005.2000',note:'MEP rough-in ongoing — crew needs through end of month'}
  ];
  const rows=items.map((r,i)=>'<div style="border:1px solid var(--gray-200);border-radius:8px;padding:12px;margin-bottom:10px" id="rcert-'+i+'">'
    +'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">'
    +'<div><div style="font-size:13px;font-weight:600">'+r.item+'</div>'
    +'<div style="font-size:11.5px;color:var(--gray-500)">'+r.id+' · on-rent '+r.on+' · <span class="mono" style="font-size:10.5px">'+r.code+'</span></div></div>'
    +'<span class="chip ok">On-rent</span></div>'
    +'<div style="font-size:11.5px;color:var(--charcoal-700);margin-bottom:10px">'+r.note+'</div>'
    +'<div style="display:flex;gap:8px">'
    +'<button class="btn sm primary" id="rcbtn-'+i+'" onclick="recertConfirm('+i+',\''+r.id+'\')">'+(ic('check','ic-14'))+' Still needed</button>'
    +'<button class="btn sm" onclick="closeModal();openOffRent(\''+r.item+'\')">'+(ic('close','ic-14'))+' Call off</button>'
    +'</div></div>').join('');
  openModal('<div class="modal-head"><div><h3>Weekly on-rent recertification</h3><div class="sub">Hercules Solar + BESS · Jul 21–25 · 3 items pending</div></div><button class="x-btn" onclick="closeModal()">'+ic('close','ic-16')+'</button></div>'
    +'<div class="modal-body"><div style="font-size:12px;color:var(--charcoal-700);margin-bottom:14px;padding:8px 12px;background:var(--gold-050);border:1px solid var(--gold-mid);border-radius:6px">'+ic('warning','ic-14')+' Confirm each item is still needed. Items confirmed will auto-renew; items called off will trigger return logistics through 02S.</div>'
    +rows+'</div>'
    +'<div class="modal-foot"><button class="btn" onclick="closeModal()">Close</button>'
    +'<button class="btn primary" onclick="closeModal();toast(\'Recertification submitted — 02S notified · 3 items renewed\')">'+ic('check','ic-14')+' Submit all &amp; renew</button></div>',true);}
function recertConfirm(idx,id){const btn=document.getElementById('rcbtn-'+idx);if(!btn)return;btn.innerHTML=ic('check','ic-14')+' Confirmed';btn.disabled=true;btn.style.opacity='.7';toast(id+' — recertified for another week');}
...
function openAssetAssign(item,qty){
  ...
  const catCls=nm.includes('crane')||nm.includes('telehandler')?'Lifting':nm.includes('excavator')||nm.includes('loader')||nm.includes('dozer')?'Earthmoving':nm.includes('scissor')||nm.includes('boom')||nm.includes('aerial')?'Aerial':'Lifting';
  const utilTarget=UTIL_TARGETS[catCls]||65;
  const vItems=ns?[
    {id:'TC-001',nm:'Liebherr LTM 1100-4.1',avail:'Available · Las Vegas yard',ovr:'Owned',utilPct:34,rec:true},
    {id:'TC-002',nm:'Grove GMK5150L',avail:'Available · Phoenix yard (3-day transit)',ovr:'Owned',utilPct:58,rec:false},
    {id:'TC-RR-1',nm:'Re-rent — Maxim Crane (quote in)',avail:'Est. $12,400/mo · 2-day lead',ovr:'Re-rent',utilPct:null,rec:false}
  ]:[
    {id:'TC-001',nm:'Liebherr LTM 1100-4.1',avail:'Las Vegas yard · 80% ready',utilPct:34,tag:''},
    {id:'TC-002',nm:'Grove GMK5150L',avail:'Phoenix yard · needs service',utilPct:58,tag:'service'},
    {id:'TC-003',nm:'Liebherr LTM 1200-9.1',avail:'Currently on Project Beta (avail Sep)',utilPct:72,tag:'in-use'}
  ];
  const costComp='<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px">'
    +'<div style="border:2px solid var(--ok);border-radius:8px;padding:12px;background:var(--ok-050)">'
    +'<div style="font-size:10px;color:var(--ok);font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">'+ic('check','ic-14')+' Owned fleet</div>'
    +'<div style="font-size:22px;font-weight:700;color:var(--charcoal)">$8,400<span style="font-size:12px;font-weight:400;color:var(--gray-500)">/mo</span></div>'
    +'<div style="font-size:11.5px;color:var(--charcoal-700);margin-top:2px">+ $1,200 one-time mob · TC-001, Las Vegas</div>'
    +'<div style="font-size:11px;color:var(--ok);margin-top:8px;font-weight:600">34% util · below '+utilTarget+'% target — idle candidate</div>'
    +'</div>'
    +'<div style="border:1px solid var(--gray-200);border-radius:8px;padding:12px">'
    +'<div style="font-size:10px;color:var(--gray-500);font-weight:700;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">'+ic('truck','ic-14')+' Re-rent</div>'
    +'<div style="font-size:22px;font-weight:700;color:var(--charcoal)">$12,400<span style="font-size:12px;font-weight:400;color:var(--gray-500)">/mo</span></div>'
    +'<div style="font-size:11.5px;color:var(--charcoal-700);margin-top:2px">Maxim Crane · 2-day lead time</div>'
    +'<div style="font-size:11px;color:var(--gray-500);margin-top:8px">+$4,000/mo vs. owned · no mob cost</div>'
    +'</div></div>'
    +'<div style="font-size:11px;color:var(--gray-500);margin-bottom:12px;padding:6px 10px;background:var(--gray-100);border-radius:6px">'+catCls+' utilization target: <b>'+utilTarget+'%</b> · Units below target are flagged as idle — policy requires own-fleet consideration first</div>';
  const rowsNS=vItems.map(a=>'<div class="lrow" style="grid-template-columns:auto 1fr 1fr .8fr auto;align-items:center;gap:10px;cursor:pointer" onclick="assignAsset(\''+item+'\',\''+a.id+'\',\''+a.nm+'\')">'
    +'<input type="radio" name="assetpick" '+(a.rec?'checked':'')+'>'
    +'<div><div style="font-size:13px;font-weight:600">'+a.nm+'</div><div style="font-size:11.5px;color:var(--gray-500)">'+a.avail+'</div></div>'
    +'<div><span class="chip '+(a.ovr==='Owned'?'ok':'gold')+'">'+a.ovr+'</span></div>'
    +'<div style="font-size:12px;color:'+(a.utilPct!==null&&a.utilPct<utilTarget?'var(--ok)':'var(--charcoal-700)')+'">'+(a.utilPct!==null?a.utilPct+'%':'—')+(a.utilPct!==null&&a.utilPct<utilTarget?' ↓ idle':'')+' util</div>'
    +'<div style="font-size:11.5px;font-weight:600;color:'+(a.rec?'var(--ok)':'var(--gray-400)')+'\">'+(a.rec?'Recommended':'')+'</div></div>').join('');
  const rowsV1=vItems.map(a=>'<div class="lrow" style="grid-template-columns:auto 1fr 1fr .6fr auto;align-items:center;gap:10px;cursor:pointer" onclick="assignAsset(\''+item+'\',\''+a.id+'\',\''+a.nm+'\')">'
    +'<input type="radio" name="assetpick" '+(!a.tag?'checked':'')+'>'
    +'<div><div style="font-size:13px;font-weight:600">'+a.nm+'</div><div style="font-size:11.5px;color:var(--gray-500)">'+a.avail+'</div></div>'
    +'<div><span class="chip '+(!a.tag?'ok':a.tag==='service'?'warn':'gray')+'">'+(!a.tag?'Available':a.tag==='service'?'Needs service':'In use')+'</span></div>'
    +'<div style="font-size:12px;color:'+(a.utilPct<utilTarget?'var(--ok)':'var(--charcoal-700)')+'">'+a.utilPct+'%'+(a.utilPct<utilTarget?' ↓ idle':'')+'</div>'
    +'<div style="text-align:right"><button class="btn sm" onclick="event.stopPropagation();assignAsset(\''+item+'\',\''+a.id+'\',\''+a.nm+'\')">Assign</button></div></div>').join('');
  openModal(`<div class="modal-head"><div><h3>Assign assets — ${item}</h3><div class="sub">${qty} unit${qty!=='1'?'s':''} required${ns?' · suggestions from T3 EAM':' · manual assignment'}</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body">`+costComp+(ns?`<div class="ai-panel" style="margin:0 0 14px"><div class="aih"><div class="ico">${ic('sparkle','ic-16')}</div><div class="t">T3 EAM recommendation</div></div><div class="ctx" style="margin-bottom:0">Recommended <b>TC-001 (Liebherr LTM 1100-4.1)</b> — available in Las Vegas yard, 34% utilization, owned. Saves ~$4,000/mo vs. re-rent after mob cost. Confirm or choose a different asset below.</div></div>`:'')+`<div style="font-size:11px;color:var(--gray-500);margin-bottom:8px">${ns?'T3 EAM suggestions:':'Select from your available fleet — or add a re-rent if nothing fits.'}</div><div class="list">`+(ns?rowsNS:rowsV1)+`</div></div><div class="modal-foot"><button class="btn" onclick="closeModal()">Cancel</button><button class="btn primary" onclick="closeModal();toast('Asset assigned — reflected in plan and YardHub')">${ic('check','ic-14')} Confirm assignment</button></div>`,true);}
...
let EDP_CART=1;
function edpCartBadges(){const n=BROWSE_CART.filter(it=>it.pillar==='Equipment'&&it.fromPlan).length||EDP_CART;document.querySelectorAll('.edp-cart-count').forEach(b=>{b.textContent=n;b.classList.toggle('empty',n===0);});}
function addLineToCart(nm,qty){const line=EQLINES.find(l=>l.nm===nm);if(line){const ex=BROWSE_CART.find(it=>it.name===nm&&it.fromPlan);if(ex){ex.qty+=line.qty;}else{BROWSE_CART.push({name:line.nm,qty:line.qty,pillar:'Equipment',quote:true,code:line.code,dates:MONTHS[line.start]+'–'+MONTHS[Math.min(7,line.start+line.span-1)],fromPlan:true});}}else{EDP_CART++;}updateCartBadges();edpCartBadges();toast(qty+' '+nm+' added to cart');}
function addGroupToCart(key,n){const added=EQLINES.filter(l=>l.code.startsWith(key.slice(0,2))||l.cat===key||l.code===key);if(added.length){added.forEach(l=>{const ex=BROWSE_CART.find(it=>it.name===l.nm&&it.fromPlan);if(!ex)BROWSE_CART.push({name:l.nm,qty:l.qty,pillar:'Equipment',quote:true,code:l.code,dates:MONTHS[l.start]+'–'+MONTHS[Math.min(7,l.start+l.span-1)],fromPlan:true});});}else{EDP_CART+=n;}updateCartBadges();edpCartBadges();toast(n+' line'+(n!==1?'s':'')+' added to cart');}
function edpSendRequest(){const planItems=BROWSE_CART.filter(it=>it.fromPlan);if(!planItems.length&&EDP_CART===0){toast('Cart is empty — add lines from the plan first');return;}
 const sample=planItems.length?planItems:EQLINES.slice(0,Math.min(EDP_CART,EQLINES.length)).map(l=>({name:l.nm,qty:l.qty,dates:MONTHS[l.start]+'–'+MONTHS[Math.min(7,l.start+l.span-1)],costCode:l.code,code:l.code}));
 if(!planItems.length&&sample.length<EDP_CART)for(let i=sample.length;i<EDP_CART;i++)sample.push({name:'Equipment line '+(i+1),dates:'Per plan',costCode:DEFAULT_COST_CODE,code:DEFAULT_COST_CODE});
...
function openAddDemand(){openModal(`<div class="modal-head"><div><h3>Add demand</h3><div class="sub">Choose how to add equipment to this plan</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body"><div style="display:flex;flex-direction:column;gap:10px"><button class="cust-bar" onclick="closeModal();addEq()" style="text-align:left"><span class="cb-ico">${ic('plus','ic-20')}</span><div class="cb-text"><div class="cb-t">Single-line entry</div><div class="cb-s">Describe one item — AI recommends taxonomy, task code, and dates from the schedule</div></div>${ic('chevronRight','ic-16')}</button><button class="cust-bar" onclick="closeModal();openBulkGrid()" style="text-align:left"><span class="cb-ico">${ic('clipboard','ic-20')}</span><div class="cb-text"><div class="cb-t">Bulk grid</div><div class="cb-s">Paste or type multiple lines at once — spreadsheet-style entry with auto-fill</div></div>${ic('chevronRight','ic-16')}</button><button class="cust-bar" onclick="closeModal();openHeavyBidImport()" style="text-align:left"><span class="cb-ico">${ic('file','ic-20')}</span><div class="cb-text"><div class="cb-t">Import from HeavyBid / estimate</div><div class="cb-s">Upload a HeavyBid extract or Excel template — 02S maps equipment lines and pre-populates the plan</div></div>${ic('chevronRight','ic-16')}</button></div></div>`);}
function openBulkGrid(){openModal(`<div class="modal-head"><div><h3>Bulk grid entry</h3><div class="sub">Add multiple lines at once</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body co-body"><div style="font-size:11.5px;color:var(--gray-500);margin-bottom:10px">Enter one line per row. Cost code and taxonomy are auto-recommended after you save.</div><table class="mini" style="padding:4px 10px"><tr><th>Equipment description</th><th>Qty</th><th>On-rent</th><th>Off-rent</th><th>Task code</th></tr>${[['Excavator 45–55T','2','Jun 2026','Dec 2026','31-2100'],['Tower crane','1','Aug 2026','Oct 2026','05-1000'],['Telehandler 10K','2','Aug 2026','Dec 2026','05-1200']].map(r=>`<tr>${r.map((v,i)=>`<td><input value="${v}" style="width:100%;font-size:12px;border:1px solid var(--gray-200);border-radius:4px;padding:3px 6px"></td>`).join('')}</tr>`).join('')}<tr>${Array(5).fill('<td><input placeholder="—" style="width:100%;font-size:12px;border:1px solid var(--gray-200);border-radius:4px;padding:3px 6px"></td>').join('')}</tr></table></div><div class="modal-foot"><button class="btn" onclick="closeModal()">Cancel</button><button class="btn primary" onclick="closeModal();toast('3 lines added to plan — taxonomy being confirmed')">${ic('check','ic-14')} Add to plan</button></div>`,true);}
function openHeavyBidImport(){openModal(`<div class="modal-head"><div><h3>Import from HeavyBid / estimate</h3><div class="sub">Pre-populate the equipment plan from an estimate extract</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body co-body"><div class="ai-panel" style="margin:0 0 14px"><div class="aih"><div class="ico">${ic('sparkle','ic-16')}</div><div class="t">What 02S will do</div></div><div class="ctx" style="margin-bottom:0">02S reads the equipment rows from your HeavyBid extract, maps each to taxonomy (Major › Minor › Class), assigns task codes from the CMC master list, and drafts on/off-rent dates from the schedule. You review and confirm before anything is submitted.</div></div><div class="field"><label>Upload file <span class="req">*</span></label><div class="dropzone" onclick="toast('File picker (demo)')">${ic('file','ic-20')}<span>Drop your HeavyBid export or McCarthy estimate template here, or click to browse</span><span style="font-size:11px;color:var(--gray-400)">.xlsx · .csv · HeavyBid export</span></div></div><div class="field"><label>Map phase codes to</label><select><option>Schedule activities (P6 import)</option><option>CMC Task code hierarchy</option><option>Manual assignment after import</option></select></div></div><div class="modal-foot"><button class="btn" onclick="closeModal()">Cancel</button><button class="btn primary" onclick="closeModal();toast('Import processing — 02S will map lines and return a draft for review')">${ic('send','ic-14')} Import &amp; map</button></div>`,true);}
function toggleRentals(on){document.querySelectorAll('#edp-schedule .gbar:not(.gbar-agg)').forEach(b=>{const st=b.dataset.status;if(on){b.style.opacity=st==='on-rent'?'1':'.18';b.style.outline=st==='on-rent'?'2px solid var(--ok)':'none';b.style.outlineOffset=st==='on-rent'?'1px':'0';}else{b.style.opacity='';b.style.outline='';b.style.outlineOffset='';}});}
...
function coSubmitPlan(n,title){const reqId='REQ-'+Math.floor(1000+Math.random()*9000);closeModal();openModal(`<div class="modal-head"><div><h3>Request submitted</h3><div class="sub">${title} · ${reqId}</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body"><div class="ai-panel" style="margin:0 0 14px"><div class="aih"><div class="ico">${ic('check','ic-16')}</div><div class="t">${n} line${n!==1?'s':''} sent to 02S</div></div><div class="ctx" style="margin-bottom:0">Your request is logged. 02S will confirm availability and return quotes within 1 business day. You'll receive a notification here and by email.</div></div><div class="list"><div class="lrow lhead" style="grid-template-columns:1fr 1fr 1fr"><div>ID</div><div>Status</div><div>Next step</div></div><div class="lrow" style="grid-template-columns:1fr 1fr 1fr"><div class="mono">${reqId}</div><div><span class="chip warn">Pending review</span></div><div style="font-size:12px;color:var(--gray-500)">02S confirms &amp; quotes</div></div></div></div><div class="modal-foot"><button class="btn primary" onclick="closeModal()">Done</button></div>`,true);}
...
const UTIL_TARGETS={Lifting:70,Earthmoving:65,Aerial:75,'Light Vehicle':60,Support:60};
const OVR_OVERRIDES={};
...
function showActionPlan(){const steps=[
 ...
 </div><div class="modal-foot"><button class="btn" onclick="closeModal()">Not now</button><button class="btn primary" onclick="assignActionPlan()">${ic('send','ic-14')} Assign all &amp; track</button></div>`,true);}
function assignActionPlan(){const actions=[
  {task:'Pull the tower-crane buy forward to Q3',pillar:'Equipment',pillarColor:'var(--red)',owner:'Fleet ops',due:'Aug 3'},
  {task:'Move headwall pre-builds Tucson → Phoenix',pillar:'Prefab',pillarColor:'var(--prefab)',owner:'Prefab shop',due:'This week'},
  {task:'Hold Nov commissioning slot for Mercy',pillar:'Prof. services',pillarColor:'var(--profsvc)',owner:'Specialist desk',due:'Fri'},
  {task:'Build the laydown business case',pillar:'Logistics',pillarColor:'var(--logistics)',owner:'Logistics lead',due:'2 weeks'},
  {task:'Stage long-lead EV chargers at the yard',pillar:'Procurement',pillarColor:'var(--proc)',owner:'Procurement',due:'On yard open'},
  {task:'Off-rent 2 idle units',pillar:'Equipment',pillarColor:'var(--red)',owner:'Fleet ops',due:'Now'},
  {task:'Renegotiate the over-rate re-rent',pillar:'Equipment',pillarColor:'var(--red)',owner:'Fleet ops',due:'This week'}
];
 const rows=actions.map((a,i)=>`<div class="lrow" style="grid-template-columns:2.5fr 1fr 1fr 1fr;align-items:center">
   <div style="font-size:13px;font-weight:500">${a.task}</div>
   <div><span class="ap-pill" style="background:${a.pillarColor}1a;color:${a.pillarColor}"><span class="pdot" style="background:${a.pillarColor}"></span>${a.pillar}</span></div>
   <div style="font-size:12px;color:var(--gray-500)">${a.owner}</div>
   <div style="display:flex;align-items:center;gap:8px"><span class="chip ok" style="font-size:10px">Assigned</span><span style="font-size:11.5px;color:var(--gray-500)">${ic('clock','ic-14')} ${a.due}</span></div>
 </div>`).join('');
 closeModal();
 openModal(`<div class="modal-head"><div><h3>Action plan — live</h3><div class="sub">7 items · assigned ${new Date().toLocaleDateString('en-US',{month:'short',day:'numeric'})} · you're the approver</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body"><div class="ai-panel" style="margin:0 0 14px"><div class="aih"><div class="ico">${ic('sparkle','ic-16')}</div><div class="t">SLA tracking started · owners notified</div></div><div class="ctx" style="margin-bottom:0">Each owner received an in-app task. You'll be flagged if any item slips past its due date.</div></div><div class="list" style="max-height:360px;overflow-y:auto"><div class="lrow lhead" style="grid-template-columns:2.5fr 1fr 1fr 1fr"><div>Action</div><div>Pillar</div><div>Owner</div><div>Due · Status</div></div>${rows}</div></div><div class="modal-foot"><button class="btn" onclick="closeModal()">Export</button><button class="btn primary" onclick="closeModal();toast('Tracking dashboard saved')">Done</button></div>`,true);}
   ...
   <div class="card span3" style="margin-bottom:16px"><div class="ch"><span class="ci">${ic('calendar','ic-16')}</span><span class="t">3-week lookahead</span><span class="sub">02S touchpoints from your V1 data</span></div>
     <div class="look">
       <div class="wk"><h4>This week <span>May 12–18</span></h4>
         ...
         <div class="item"><div class="top">${ic('warning','ic-14')} On-rent recertification due</div><div class="meta">3 items · confirm still needed by Fri · <button class="btn sm" style="padding:1px 7px;font-size:10.5px;line-height:1.4" onclick="openRecertModal()">Review</button></div></div>
...
'p-orders':(id)=>`
 <div class="page-head"><div><h2>Orders &amp; requests</h2>${projBadge()}</div>${prodToggle(id)}</div>
 <div style="display:flex;align-items:center;gap:12px;margin:14px 0 0;padding:12px 16px;background:var(--gold-050);border:1px solid var(--gold-mid);border-radius:8px">
   <span style="color:var(--gold);flex-shrink:0">${ic('warning','ic-16')}</span>
   <div style="flex:1"><div style="font-size:13px;font-weight:600;color:var(--charcoal)">3 items due for on-rent recertification this week</div><div style="font-size:11.5px;color:var(--charcoal-700)">Confirm still needed or initiate call-off by Fri, Jul 24 — 02S will renew or begin return logistics accordingly</div></div>
   <button class="btn sm" style="flex-shrink:0" onclick="openRecertModal()">Review items</button>
 </div>
 ...
 <div class="filter-bar" style="display:flex;gap:8px;flex-wrap:wrap;margin:10px 0;align-items:center">
   <select class="btn" style="font-size:12px"><option>All statuses</option><option>New</option><option>Acknowledged</option><option>Fulfilled</option><option>Unable to Fill</option><option>Updated</option><option>Archived</option><option>On-rent</option><option>Off-rent</option></select>
   <select class="btn" style="font-size:12px"><option>All pillars</option><option>Equipment</option><option>Prefab</option><option>Procurement</option><option>Logistics</option><option>Prof. services</option></select>
   <select class="btn" style="font-size:12px"><option>All request types</option><option>Call-on</option><option>Call-off</option><option>Catalog order</option><option>Plan request</option></select>
   <input placeholder="Task code / cost code…" style="font-size:12px;border:1px solid var(--gray-200);border-radius:6px;padding:5px 10px;min-width:160px">
   <input placeholder="Search orders…" style="font-size:12px;border:1px solid var(--gray-200);border-radius:6px;padding:5px 10px;flex:1;min-width:160px">
   <input type="date" style="font-size:12px;border:1px solid var(--gray-200);border-radius:6px;padding:5px 8px" title="From date">
   <input type="date" style="font-size:12px;border:1px solid var(--gray-200);border-radius:6px;padding:5px 8px" title="To date">
 </div>
 <div id="ord-active" class="list">
   <div class="lrow lhead" style="grid-template-columns:1fr 1.5fr 1.2fr 1fr 1fr .8fr"><div>Order / Req</div><div>Product</div><div>Dates</div><div>Task code</div><div>Job/Dept</div><div>Status</div></div>
   ${ORDERS.filter(o=>o.stage<5).map((o,i)=>{const sc=o.status==='On-rent'?'ok':o.status==='Fulfilled'?'ok':o.status==='Acknowledged'?'warn':o.status==='Unable to Fill'?'red':'gray';return `<div class="lrow click" style="grid-template-columns:1fr 1.5fr 1.2fr 1fr 1fr .8fr" onclick="openOrderDetail(${ORDERS.indexOf(o)})"><div class="pri">${o.id} ${ic('chevronRight','ic-14')}</div><div>${o.prod}</div><div style="font-size:12px">${o.dates}</div><div class="mono" style="font-size:11px">${o.code}</div><div style="font-size:12px;color:var(--gray-500)">004987.01</div><div><span class="chip ${sc}">${o.status}</span></div></div>`;}).join('')}
 </div>
 <div id="ord-history" class="list hidden">
   <div class="lrow lhead" style="grid-template-columns:1fr 1.5fr 1.2fr 1fr 1fr .8fr"><div>Order / Req</div><div>Product</div><div>Arrived</div><div>Task code</div><div>Job/Dept</div><div>Status</div></div>
   ${ORDERS.map((o,i)=>{const sc=o.status==='Off-rent'||o.status==='Archived'?'gray':o.status==='On-rent'||o.status==='Fulfilled'?'ok':'warn';return `<div class="lrow click" style="grid-template-columns:1fr 1.5fr 1.2fr 1fr 1fr .8fr" onclick="openOrderDetail(${i})"><div class="pri">${o.id} ${ic('chevronRight','ic-14')}</div><div>${o.prod}</div><div style="font-size:12px">${o.arrived}</div><div class="mono" style="font-size:11px">${o.code}</div><div style="font-size:12px;color:var(--gray-500)">004987.01</div><div><span class="chip ${sc}">${o.status}</span></div></div>`;}).join('')}
 </div>
 <div style="font-size:11.5px;color:var(--gray-500);margin-top:12px">Click any row to see order detail — dates, cost code, YardHub ID, and transfer date. ${'{ns}'}</div>
...
'p-edp':(id)=>`
 ...
 <div class="edp-toolbar">
   <button class="btn primary" onclick="openAddDemand()">${ic('plus','ic-14')} Add demand</button>
   <select class="btn" style="font-weight:600" onchange="setEDPGroup(this.value)"><option>Group by: Task code</option><option>Group by: Equipment class</option><option>Group by: Scope</option></select>
   <label style="display:flex;align-items:center;gap:6px;font-size:12px;cursor:pointer;white-space:nowrap"><input type="checkbox" id="rentalsToggle" onchange="toggleRentals(this.checked)"> On-rent overlay</label>
 ...
 <div id="edp-schedule">${buildSchedule('Task code')}</div>
 <div id="edp-list" class="hidden">${buildList('Task code')}</div>
 ...
 <div class="card" style="margin-top:16px"><div class="ch"><span class="ci">${ic('send','ic-16')}</span><span class="t">Requests against this plan</span><span class="sub">plan is the projection · a request is the actual call-on</span><button class="btn primary sm v1-only" style="margin-left:auto" onclick="edpSendRequest()">${ic('send','ic-14')} Send request <span class="cart-badge edp-cart-count">1</span></button></div>
   <div class="list"><div class="lrow lhead" style="grid-template-columns:1fr 1.5fr 1fr 1fr 1fr"><div>Request</div><div>Equipment</div><div>Type</div><div>Submitted</div><div>Status</div></div>
   <div class="lrow click" style="grid-template-columns:1fr 1.5fr 1fr 1fr 1fr" onclick="nav('portal','p-orders')"><div class="pri">REQ-1042</div><div>2× ¾-Ton 4×4</div><div>Call-on</div><div>May 12</div><div><span class="chip ok">Fulfilled</span></div></div>
   <div class="lrow click" style="grid-template-columns:1fr 1.5fr 1fr 1fr 1fr" onclick="nav('portal','p-orders')"><div class="pri">REQ-1051</div><div>1× SUV AWD</div><div>Call-on</div><div>May 14</div><div><span class="chip warn">Acknowledged</span></div></div>
   <div class="lrow click" style="grid-template-columns:1fr 1.5fr 1fr 1fr 1fr" onclick="nav('portal','p-orders')"><div class="pri">REQ-1058</div><div>2× excavator 45–55T</div><div>Call-on</div><div>—</div><div><span class="chip gray">New · in cart</span></div></div></div>
