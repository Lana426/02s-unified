...
function pillarRequestValidate(name,pc,ns){
 ...
 openModal(`<div class="modal-head"><div><h3>Submit ${name} request</h3><div class="sub">${sub}</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body">${codeBlock}</div><div class="modal-foot"><button class="btn" onclick="closeModal()">Keep editing</button><button class="btn primary" onclick="closeModal();toast('${name} request submitted — appears in 02S backlog')">${ic('send','ic-14')} Submit request</button></div>`,true);}
