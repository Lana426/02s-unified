...
function openOffRent(){openModal(`<div class="modal-head"><div><h3>Initiate off-rent</h3><div class="sub">Selected lines</div></div><button class="x-btn" onclick="closeModal()">${ic('close','ic-16')}</button></div><div class="modal-body co-body"><div class="co-note">${ic('warning','ic-14')} This will call off the selected equipment lines and trigger return logistics.</div><div class="field-row"><div class="field"><label>Off-rent date <span class="req">*</span></label><input type="date" id="orDate" value="${new Date(Date.now()+86400000*3).toISOString().slice(0,10)}"></div><div class="field"><label>Return to</label><select><option>02S West yard — Las Vegas</option><option>02S Central depot</option><option>On-site (owner pickup)</option></select></div></div><div class="field"><label>Notes <span style="font-weight:400;color:var(--gray-400)">optional</span></label><textarea rows="2" placeholder="Any return instructions or contacts…"></textarea></div></div><div class="modal-foot"><button class="btn" onclick="closeModal()">Cancel</button><button class="btn primary" onclick="closeModal();toast('Off-rent initiated — return logistics triggered')">${ic('check','ic-14')} Confirm off-rent</button></div>`);}
