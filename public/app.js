...
function openCheckout(items,opts){
 ...
 const itemsHTML=items.map(it=>{
   ...
   return `<div class="co-line"><span class="co-nm">${it.name}${it.qty&&it.qty>1?` <span class="co-qty">×${it.qty}</span>`:''}</span><span class="co-right">${it.dates?`<span class="co-dates">${it.dates}</span>`:''}${amt?`<span class="co-amt">${amt}</span>`:''}</span></div>`;
 }).join('');
